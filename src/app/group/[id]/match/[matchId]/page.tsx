...
import { getMatch, getGroup } from "@/lib/store";
...
export default function MatchDetailPage() {
  ...
  useEffect(() => {
    setMatch(getMatch(groupId, matchId));
  ...
  if (!match) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <p className="text-text-muted">경기를 찾을 수 없습니다</p>
      </div>
    );
  }
  ...
  const getMemberRealName = (nickname: string) => {
    const m = members.find((m) => m.nickname === nickname);
    return m?.realName;
  };
      ...
      <Card className="mb-6 p-0 overflow-hidden">
        <div className="bg-gradient-to-r from-blue-500/5 via-transparent to-red-500/5 p-6">
          ...
          <div className="flex items-center justify-center gap-3 mb-5">
            ...
            {match.gameDuration && (
              <span className="text-xs text-text-muted px-2 py-0.5 rounded-full bg-bg-secondary">
                {match.gameDuration}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center justify-center gap-6 sm:gap-10">
            ...
            <div className="text-center">
              <div className={`text-4xl sm:text-5xl font-extrabold tabular-nums ${blueWin ? "text-blue-400" : "text-text-muted/60"}`}>
                {blueKills}
              </div>
            ...
            <div className="flex flex-col items-center gap-1">
              <Swords size={24} className="text-text-muted/30" />
              <span className="text-xs font-bold text-text-muted/40">VS</span>
            </div>

            {/* Red */}
            <div className="text-center">
              <div className={`text-4xl sm:text-5xl font-extrabold tabular-nums ${!blueWin ? "text-red-400" : "text-text-muted/60"}`}>
                {redKills}
              </div>
            ...
            <div className="flex items-center justify-center gap-3 mt-5">
              {mvp && (
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-mvp/10 border border-mvp/20">
                  ...
                  <span className="text-sm font-bold text-text-primary">{mvp.nickname}</span>
                  <span className="text-xs text-text-muted">{mvp.mvpScore.toFixed(1)}점</span>
                </div>
              )}
              {ace && (
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-ace/10 border border-ace/20">
                  ...
                  <span className="text-sm font-bold text-text-primary">{ace.nickname}</span>
                  <span className="text-xs text-text-muted">{ace.mvpScore.toFixed(1)}점</span>
                </div>
              )}
            </div>
          )}
...
function TeamSection({
  ...
  maxTaken: number;
  maxGold: number;
  getRealName: (nickname: string) => string | undefined;
}) {
  ...
  return (
    <Card className={`border-l-4 ${borderColor} p-0 overflow-hidden`}>
      {/* Team Header */}
      <div className={`${headerBg} px-4 py-2.5 flex items-center justify-between border-b border-border/50`}>
        <div className="flex items-center gap-2">
          <span className={`text-sm font-bold ${teamColor}`}>
            {team === "blue" ? "블루팀" : "레드팀"}
          </span>
          <Badge variant={win ? "win" : "lose"} className="text-[10px]">
            {win ? "승리" : "패배"}
          </Badge>
        </div>
        <div className="text-xs text-text-muted">
          {players.reduce((s, p) => s + p.kills, 0)}/
          {players.reduce((s, p) => s + p.deaths, 0)}/
          {players.reduce((s, p) => s + p.assists, 0)}
        </div>
      </div>
      ...
      <div className="divide-y divide-border/30">
        {players.map((p) => {
          ...
          return (
            <div key={p.id} className="px-4 py-3 hover:bg-bg-card-hover/30 transition-colors">
              {/* Row 1: Player info + KDA */}
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2.5">
                  {/* Champion initial */}
                  <div className="w-9 h-9 rounded-xl bg-bg-secondary flex items-center justify-center text-sm font-bold text-text-muted shrink-0">
                    {p.champion ? p.champion.charAt(0) : "?"}
                  </div>
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className="font-semibold text-text-primary text-sm">{p.nickname}</span>
                      {realName && <span className="text-xs text-text-muted">({realName})</span>}
                      {p.isMvp && <Badge variant="mvp" className="text-[10px] px-1.5 py-0">MVP</Badge>}
                      {p.isAce && <Badge variant="ace" className="text-[10px] px-1.5 py-0">ACE</Badge>}
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-text-muted">
                      <span>{p.champion}</span>
                      {p.lane && (
                        <span>{LANE_EMOJI[p.lane]} {LANE_LABEL[p.lane]}</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 text-right">
                  ...
                  <div className="w-10 text-center">
                    <div className="text-sm font-bold text-accent">{p.mvpScore.toFixed(1)}</div>
              ...
              <div className="grid grid-cols-3 gap-3 ml-[46px]">
                ...
                <div>
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="text-text-muted">CS / 골드</span>
                    <span className="text-text-primary tabular-nums">{p.cs}</span>
                  </div>
                  <div className="h-1.5 bg-bg-secondary rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-yellow-500/60 to-yellow-400 rounded-full"
                ...
                <div>
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="text-text-muted flex items-center gap-0.5"><Flame size={10} />딜량</span>
                    <span className="text-text-primary tabular-nums">{(p.damageDealt || 0).toLocaleString()}</span>
                  </div>
                  <div className="h-1.5 bg-bg-secondary rounded-full overflow-hidden">
                    <div
                ...
                <div>
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="text-text-muted flex items-center gap-0.5"><Shield size={10} />피해</span>
                    <span className="text-text-primary tabular-nums">{(p.damageTaken || 0).toLocaleString()}</span>
                  </div>
                  <div className="h-1.5 bg-bg-secondary rounded-full overflow-hidden">
                    <div
              ...
              {/* Row 3: Extra stats (if available) */}
              {(p.visionScore > 0 || p.wardsPlaced > 0) && (
                <div className="flex items-center gap-4 ml-[46px] mt-1.5 text-xs text-text-muted">
                  {p.visionScore > 0 && (
                    <span className="flex items-center gap-0.5">
                      <Eye size={10} /> 시야 {p.visionScore}
                    </span>
                  )}
                  {p.wardsPlaced > 0 && (
                    <span>와드 {p.wardsPlaced}개</span>
                  )}
                  {p.wardsDestroyed > 0 && (
                    <span>제거 {p.wardsDestroyed}개</span>
                  )}
                  {p.killParticipation > 0 && (
                    <span>킬관여 {p.killParticipation.toFixed(0)}%</span>
                  )}
                </div>
              )}
            </div>
